...
fn fileExistStdoutOrCreate(self: *Self) bool {
...
fn fileExistStderrOrCreate(self: *Self) bool {
...
pub fn start(self: *Self,  program: *Program, exec: []const []const u8, process_program: *ProcessProgram) !*std.Thread {
    ...
    if (pid == 0) {
        // we are the child
        if (self.fileExistStderrOrCreate()) self.*.stderr_file = try std.fs.openFileAbsolute(self.*.stderr_path, .{ .mode = .read_write });
        if (self.fileExistStdoutOrCreate()) self.*.stdout_file = try std.fs.openFileAbsolute(self.*.stdout_path, .{ .mode = .read_write });
